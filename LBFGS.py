...
# ==========================================================================
def LBFGS(w_init, X, y, seed, numIter, mmr, radius, eps, alpha_init, cArmijo, rhoArmijo, num_weights,
            init_sampling_SLBFGS, dnn, sess):
    ...
    while k<100:
        ...
        S_buffer.append(-alpha * pk)
        S = np.squeeze(S_buffer).T

        Y_buffer.append(g_k_newTemp[0] - g_k)
    ...
    pickle.dump(HISTORY, open("./_saved_log_files/LBFGS.pkl", "wb"))  # Save History in .pkl file
    # pickle.dump( weights_SLBFGS, open( "./_saved_log_files/LBFGS_weights.pkl", "wb" ) )    # Save Weights in .pkl file
